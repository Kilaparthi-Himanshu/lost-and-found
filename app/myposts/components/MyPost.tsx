...
export const MyPost = ({ post }: PostProps) => {
    ...
    return (
        <>
            <div className='border border-neutral-400 dark:border-neutral-600 dark:hover:bg-neutral-800 h-full w-full rounded-lg p-2 flex max-xl:flex-col gap-2 bg-neutral-100 dark:bg-neutral-900 hover:bg-sky-100 cursor-pointer transition-[background,scale] hover:scale-101 relative xl:max-h-100 shadow-md' onClick={handleClick}>
                <img
                    className='rounded-lg object-cover h-full max-xl:max-h-80 max-h-100 max-w-[400px] max-xl:max-w-full shadow-lg'
                    src={post.img}
                    alt="Picture of the author"
                />
                <div className='ml-2 border-l-2 rounded-l-none border-neutral-400 dark:border-neutral-600 flex-1 rounded-lg max-sm:py-2 p-4 font-medium flex flex-col h-full text-lg justify-between gap-2 w-full'>
                    ...
                    <div className='absolute w-auto max-w-20 h-max gap-2 right-3 bottom-3 flex items-center justify-around max-sm:relative max-sm:mt-4 sm:m-2 p-1 rounded'>
